...
- Documented the optional Electron GUI host (`packages/gui` / `@bastani/atomic-gui`) and its interactive-engine protocol alongside the standalone TUI (`docs/gui.md`).
...
- Added engine-owned GUI RPCs for session sharing, deletion, renaming, durable headers, project trust, model authentication capabilities, external-editor lookup, Codex fast mode, and validated settings mutations. Electron never receives raw settings, trust, or credential files.
- Added typed engine-owned theme snapshots and mutations for the optional GUI host, including resolved CSS tokens and settings-precedence metadata.
- Added cancellable engine-owned autocomplete queries and ordered `ctx.ui.onTerminalInput` interception for the optional interactive-engine GUI host, without changing the standalone TUI input loop.
- Optional GUI transcript hosts now support safe raster attachments and escaped unified-diff tool output, while renderer theme and model-scope changes remain engine-validated.
- Named `ctx.ui.setTheme(...)` selections now propagate through the isolated interactive host and resolve renderer CSS in the engine without exposing theme files or raw CSS to optional GUI clients.
...
- Fixed optional GUI `ctx.ui.editor` dialogs ignoring declared timeout and abort-signal options; isolated interactive extensions now receive the same cancellation contract as select, confirm, and input dialogs.
- Fixed optional GUI settings batches applying an earlier valid operation before a later operation failed validation; the engine now validates the complete batch before persisting changes.
- Fixed optional GUI-host `@` completion to use the same engine fallback provider as the TUI when the primary autocomplete chain has no result.
