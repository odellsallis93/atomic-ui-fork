...
export interface AtomicCommand {
	readonly command: "atomic";
	readonly auth?: AuthInput;
	readonly options: Args;
	readonly listen?: readonly TransportAddress[];
}

export interface AtomicCommandContext {
	runAtomic(command: AtomicCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const atomicCommand = new Command<AtomicCommand, AtomicCommandContext>("atomic")
	.option(listenOption)
	...
	.action((command, context) => context.runAtomic(command));
