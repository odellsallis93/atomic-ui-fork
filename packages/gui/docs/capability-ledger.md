...
| MCP walkthrough | `/mcp`, `/mcp setup`, and `/mcp-auth` use generic `engine_custom_*` frames; MCP and direct tool calls use generic tool events/rendering | `electron-phase2.e2e.test.ts` MCP OAuth Ctrl+C and tool-render cases; `packages/mcp/commands.ts`; `packages/mcp/index.ts`; `agent-events.ts` | fixture E2E + source | partial — renderer-host MCP login cancellation and tool presentation are proven. Real configured-server OAuth and calls remain engine-owned and unproven. |
| Other bundled extensions (workflows/subagents/intercom/web) | Generic frames only | — | — | open (Phase 4 / M6) |
...
|---|---|---|
| MCP panels and OAuth picker | Generic custom frame host (`engine_custom_open`, render, input, dispose) | `/mcp setup`, `/mcp`, and `/mcp-auth` call `ctx.ui.custom` with `handlesCtrlC: true` in `packages/mcp/commands.ts`. `FrameOverlay` forwards Ctrl+C to the focused frame; fixture E2E proves the renderer-host cancel path. |
| MCP OAuth cancellation and secrets | Engine-owned browser/callback flow; no MCP-specific GUI auth RPC | `mcp-auth-flow.ts` aborts and clears owned state on reset; token storage stays in `mcp-auth.ts`. Provider OAuth envelopes in `rpc-types.ts` are separate. The GUI does not read, log, write, or return credentials. |
| MCP proxy and direct tool use | Generic `tool_execution_*` events → `ToolRenderHost` → `engine_tool_render` | MCP registers the `mcp` proxy plus optional direct tools in `packages/mcp/index.ts`; `agent-events.ts` defines the generic event shapes. Fixture E2E proves the renderer-host transcript/render loop only. |
