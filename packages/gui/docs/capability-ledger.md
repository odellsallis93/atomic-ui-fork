...
| `test/electron-phase2.e2e.test.ts` | fixture E2E | Built Electron host ready state and CodeMirror focus | Queue, sessions, tree, and compaction semantics; see `docs/e2e-harness.md` |
...
| Queue / steer / follow-up chips | Composer queue UI | `composer-parity.test.ts`; `queue_update` store tests | unit | partial — dequeue restores queue text FIFO and keeps the engine pause hold; real-engine/E2E proof remains blocked by the native-binding failure recorded in `docs/e2e-harness.md` |
...
| Sessions list/resume/rename/delete/clone/export/compact/tree | Session picker / tree modal; engine `fork`, `import_session`, `set_label`, `get_commands` | `engine-client.test.ts` routes fork/import/label and filters the executable `get_commands` inventory; `session-store.test.ts` honors active-leaf compaction boundaries; `tree-navigator.test.ts` covers folded search and active-leaf retention; runtime inventory: `rpc-command-handler.ts` | unit + source inventory | partial — fork/import/label route through existing RPC. Import preserves the JSONL cwd unless engine recovery needs an override. Share has no runtime RPC and stays excluded. The real-engine Electron attempt is blocked before fork/import/tree/compaction by the native-binding error in `docs/e2e-harness.md`; fixture E2E is not semantic proof. |
