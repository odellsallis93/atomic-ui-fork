...
| Session switch / reload integrity | Session picker → `switchSession` + `resetTranscript` + `hydrateTranscript(entries, leafId)` | `session-store.test.ts` (“session switch hydration…”); `real-engine-smoke.test.ts` (leaf alignment); `App.tsx` | unit + real-engine | partial — host replace+leaf proven; engine bash-row durability across switch is best-effort (assert when present) |
| Transcript kinds (user/assistant/tool/bash/…) | `Transcript.tsx` / `session-store` | `session-store.test.ts` | unit | partial — Phase 2 completeness |
