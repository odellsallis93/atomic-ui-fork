	...
	for (const entry of entries) {
		...
		if (entry.type === "workflow.stage.end") {
			...
			if (snap) {
				...
				if (typeof failureRecoverability === "string" && isWorkflowFailureRecoverability(failureRecoverability))
					snap.failureRecoverability = failureRecoverability;
				if (typeof failureDisposition === "string" && isWorkflowFailureDisposition(failureDisposition))
					snap.failureDisposition = failureDisposition;
...
function markRestoredBlockedFailureStage(snap: StageSnapshot, blockedMeta: RestoredRunBlockedMetadata): void {
	snap.status = "failed";
...
export function findRunBlockedMetadata(
	...
	for (const entry of entries) {
		...
		latest = {
			...
			failureRecoverability: "recoverable",
			...(typeof failureDisposition === "string" && isWorkflowFailureDisposition(failureDisposition)
				? { failureDisposition }
				: {}),
...
export function restoreTerminalRuns(entries: readonly SessionEntry[], store: Store): void {
	const started = new Map<
		string,
		{ readonly name: string; readonly inputs: Readonly<WorkflowInputValues>; readonly startTs: number }
	>();
	...
	for (const [runId, start] of started) {
		...
		store.recordRunEnd(runId, status, result, typeof error === "string" ? error : undefined, {
			...
			...(typeof failureCode === "string" && isWorkflowFailureCode(failureCode) ? { failureCode } : {}),
			...(typeof failureRecoverability === "string" && isWorkflowFailureRecoverability(failureRecoverability)
			...
			...(typeof resumable === "boolean"
				? { resumable }
				: isWorkflowExitTerminalStatus(status) && restoredAuthorExit
					? { resumable: false }
					: {}),
...
export function findRunStartMetadata(
	...
	for (const entry of entries) {
		...
		return {
			...
			...(typeof accumulatedDurationMs === "number" &&
			Number.isFinite(accumulatedDurationMs) &&
