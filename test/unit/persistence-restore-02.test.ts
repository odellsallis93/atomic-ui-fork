		...
		const entries: SessionEntry[] = [
			{ id: "e1", type: "workflow.run.start", payload: { runId: "r2", name: "wf", inputs: {}, ts: 1 } },
			{
				...
				payload: {
					...
					name: "import:child",
					parentIds: [],
					replayKey: "workflow:import:child",
					ts: 2,
				},
			},
			{
			...
			const entries: SessionEntry[] = [
				{
					id: `${status}-e1`,
					type: "workflow.run.start",
					payload: { runId: `r-${status}`, name: "wf", inputs: {}, ts: 1 },
				},
				{
					id: `${status}-e2`,
					type: "workflow.stage.start",
	...
	test("restores malformed and non-terminal stage.end statuses as failed", () => {
		...
		const entries: SessionEntry[] = [
			{ id: "e1", type: "workflow.run.start", payload: { runId: "r1", name: "wf", inputs: {}, ts: 1 } },
			{
				id: "e2",
				type: "workflow.stage.start",
				payload: { runId: "r1", stageId: "s1", name: "one", parentIds: [], ts: 2 },
			},
			{ id: "e3", type: "workflow.stage.end", payload: { runId: "r1", stageId: "s1", status: "running" } },
			{
				id: "e4",
				type: "workflow.stage.start",
				payload: { runId: "r1", stageId: "s2", name: "two", parentIds: [], ts: 3 },
			},
	...
	test("restores failed terminal run metadata from run.end entries", () => {
		...
		const entries: SessionEntry[] = [
			...
			},
			{
				id: "e3",
				type: "workflow.stage.end",
				payload: { runId: "r1", stageId: "s1", status: "failed", error: "rate limit", failureKind: "rate_limit" },
			},
			{
				id: "e4",
	...
	test("ignores invalid run failureKind from run.end entries", () => {
		...
		const entries: SessionEntry[] = [
			{ id: "e1", type: "workflow.run.start", payload: { runId: "r1", name: "wf", inputs: {}, ts: 1 } },
			{
				id: "e2",
				type: "workflow.run.end",
				payload: { runId: "r1", status: "failed", error: "boom", failureKind: "not-real", resumable: true, ts: 2 },
			},
		];
	...
	test("restores workflow.run.blocked as active recoverable state for any resumeInFlight policy", () => {
		...
		const entries: SessionEntry[] = [
			...
			},
			{
				id: "e4",
				type: "workflow.stage.start",
				payload: { runId: "r1", stageId: "s2", name: "after", parentIds: ["s1"], ts: 3 },
			},
			{
				id: "e5",
