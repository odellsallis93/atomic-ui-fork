...
import { createStore, assert as exAssert, test as exTest, run, Type, workflow } from "./executor-shared.js";
...
describe("recordStageCheckpoint", () => {
	...
	test("records failed/skipped topology metadata without successful outputs", async () => {
		for (const status of ["failed", "skipped"] as const) {
			...
			const cp = backend
				.listCheckpoints(WORKFLOW_ID)
				.find((item) => item.kind === "stage" && item.replayKey === replayKey);
	...
	test("schema-backed live stage records structured prompt output", async () => {
		...
		const result = await stage("structured", {
			schema: Type.Object({ summary: Type.String(), count: Type.Number() }),
		}).prompt("structured");
	...
	test("schema-backed replay returns structured prompt output", async () => {
		...
		const stage = createDurableStagePrimitive({
			...
			nextReplayKey: () => replayKey,
			stage: () => {
				throw new Error("live stage should not run");
			},
		});
	...
	test("replayed stage invokes recordCachedStage for graph/store visibility", async () => {
		...
		const stage = createDurableStagePrimitive({
			...
			backend,
			nextReplayKey: () => replayKey,
			recordCachedStage: (name, key, checkpoint) =>
				recorded.push({ name, replayKey: key, output: String(checkpoint.output) }),
			stage: () => {
				throw new Error("live stage should not run");
			},
		});
...
describe("run durable flush", () => {
	exTest("completed stage checkpoint is awaited before next stage starts", async () => {
		class AsyncStageBackend extends InMemoryDurableBackend {
			...
			async recordCheckpointAsync(
				checkpoint: import("../../packages/workflows/src/durable/types.js").DurableCheckpoint,
			): Promise<void> {
	...
	exTest("workflow completion waits for durable flush", async () => {
		...
		backend.recordCheckpoint({
			kind: "stage",
			workflowId: "wf-flush",
			checkpointId: "stage:stage:cached:1",
			name: "cached",
			replayKey: "stage:cached:1",
			output: "cached",
			completedAt: Date.now(),
		});
		const store = createStore();
	...
	exTest("ctx.chain and ctx.parallel replay completed task checkpoints", async () => {
		...
		const first = await run(
			def,
			{},
			{ runId: "wf-chain-parallel", store: createStore(), durableBackend: backend, adapters },
		);
		...
		const second = await run(
			...
			{},
			{
				...
				adapters: {
					complete: {
						complete: async () => {
							throw new Error("child should not rerun");
						},
					},
				},
			},
	...
	exTest("repeated ctx.workflow(child) calls replay in order without re-execution", async () => {
		...
		const child = workflow({
			...
			run: async (ctx) => {
				childRuns++;
				return { value: await ctx.stage("c").complete(`v${childRuns}`) };
			},
		});
		const parent = workflow({
	...
	exTest("ctx.exit terminal status persisted to durable metadata", async () => {
		...
		const def = workflow({
			...
			outputs: {},
			run: async (ctx) => {
				ctx.exit({ status: "cancelled", reason: "user-abort" });
			},
		});
	...
	exTest("ctx.exit blocked status persisted to durable metadata", async () => {
		...
		const def = workflow({
			...
			outputs: {},
			run: async (ctx) => {
				ctx.exit({ status: "blocked", reason: "needs-input" });
			},
		});
	...
	exTest("child workflow run propagates custom durableBackend", async () => {
		class TrackingBackend extends InMemoryDurableBackend {
			registered: string[] = [];
			registerWorkflow(
				h: import("../../packages/workflows/src/durable/backend.js").WorkflowRegistrationInput,
			): void {
		...
		const result = await run(
			...
			{},
			{
				runId: "wf-backend-prop",
				store: createStore(),
				durableBackend: backend,
				adapters: { complete: { complete: async (t) => t } },
			},
		);
		exAssert.equal(result.status, "completed");
